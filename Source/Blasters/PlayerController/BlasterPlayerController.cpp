...
#include "Blasters/BlasterTypes/Announcement.h"
...
void ABlasterPlayerController::ClientElimAnnouncement_Implementation(APlayerState *Attacker, APlayerState *Victim)
{
    ...
    if (Attacker && Victim && Self)
    {
        ...
        if (BlasterHUD)
        {
            ...
            {
                BlasterHUD->AddElimAnnouncemnet("You", Victim->GetPlayerName());
                return;
            }
            if (Victim == Self && Attacker != Self)
            {
                BlasterHUD->AddElimAnnouncemnet(Attacker->GetPlayerName(), "You");
                return;
            }
            if (Attacker == Victim && Attacker == Self)
            {
                BlasterHUD->AddElimAnnouncemnet("You", "You");
                return;
            }
            if (Attacker == Victim && Attacker != Self)
            {
                BlasterHUD->AddElimAnnouncemnet(Attacker->GetPlayerName(), "themselves");
                return;
            }
            BlasterHUD->AddElimAnnouncemnet(Attacker->GetPlayerName(), Victim->GetPlayerName());
        }
    }
}
...
void ABlasterPlayerController::SetHUDRedTeamScore(int32 RedScore)
{
    ...
    if (bHUDValid)
    {
        FString RedScoreText = FString::Printf(TEXT("%d"), RedScore);
...
void ABlasterPlayerController::SetHUDBlueTeamScore(int32 BlueScore)
{
    ...
    if (bHUDValid)
    {
        FString BlueScoreText = FString::Printf(TEXT("%d"), BlueScore);
